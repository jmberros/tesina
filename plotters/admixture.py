...
class Admixture:
    def read_cv_errors(self):
        ...
        df["panel"] = df["panel"].apply(self._fix_panel_names)
    ...
    # Ugly hack not to run admixture again
    def _fix_panel_names(self, name):
        name = name.replace("gal_affy", "GAL_Affy")
        name = name.replace("gal_completo", "GAL_Completo")
        name = name.replace("cpx", "CPx")
        return name
